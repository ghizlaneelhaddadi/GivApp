...
import com.example.test1000.databinding.ActivityEditprofilAdminBinding
...
class EditProfilAdminActivity : AppCompatActivity() {
            ...
            if (validateInputs(username, email, phone, newPassword, confirmation)) {
                selectedImageUri?.let { imageUri ->
                    uploadImageToFirebase(imageUri) { imageUrl ->
                        showConfirmationDialog(username, email, phone, newPassword , adresse,nationality,
                            imageUri.toString(),status)
    ...
    private fun validateInputs(
        ...
        confirmation : String
            ...
            if(username.isNotEmpty()&&email.isNotEmpty()&&phone.isNotEmpty()&&newPassword.equals(confirmation))
                return true
        else
        return false
    }
    ...
    private fun showConfirmationDialog(
        ...
        builder.setPositiveButton("Oui") { dialog, _ ->
            dialog.dismiss()
            updateUserProfile(username, email, phone, newPassword, adresse ,nationality,image,status)
        }
    ...
    private fun updateUserProfile(
        ...
        currentUser?.let { user ->
            ...
            userUpdates["adresse"] = adresse
            userUpdates["nationality"] = nationality
            ...
            val databaseUpdateTask = database.child("users").child(userId).updateChildren(userUpdates)

            emailUpdateTask?.addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Toast.makeText(this, "Email mis à jour avec succès", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this, "Échec de la mise à jour de l'email", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            databaseUpdateTask.addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Toast.makeText(this, "Profil modifié avec succès", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this, "Échec de la modification du profil", Toast.LENGTH_SHORT).show()
                }
            }
        } ?: run {
